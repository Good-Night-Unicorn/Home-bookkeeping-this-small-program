	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<view class="search-form round">
				<input v-model="searchForm.xiaofeixiangqingName" type="text" placeholder="标题"></input>
			</view>
		...
		<scroll-view scroll-x="true" class="tabView"
			...
			<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
				v-for="(item,index) in xiaofeixiangqingTypesList" :key="index"
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
			<view @tap="onDetailTap(product)" class="uni-product"
				...
				<view class="uni-product-price" v-if="product.xiaofeixiangqingNewMoney != null"
					:style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#999","textAlign":"left"}'>
					<text class="">￥{{product.xiaofeixiangqingNewMoney}}</text>
				</view>
				<view style="display: flex;justify-content: space-between;" v-if="product.yonghuId == user.id">
					<text v-if="isAuth('xiaofeixiangqing','修改')" class="cuIcon-edit"
						@click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('xiaofeixiangqing','删除')" class="cuIcon-delete"
			...
			v-if="isAuth('xiaofeixiangqing','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		async onShow() {
			...
			//当前表的 消费类型 字段 字典表查询方法
			let xiaofeixiangqingTypes = await this.$api.page('dictionary', {
				page: 1,
				limit: 100,
				dicCode: 'xiaofeixiangqing_types'
			});
			this.xiaofeixiangqingTypesList = xiaofeixiangqingTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.xiaofeixiangqingName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if (this.categoryName != 0) {
					params.xiaofeixiangqingTypes = this.categoryName
				}

				let res = await this.$api.list(`xiaofeixiangqing`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xiaofeixiangqing', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.xiaofeixiangqingName) {
					searchForm['xiaofeixiangqingName'] = this.searchForm.xiaofeixiangqingName
				}
				if (this.searchForm.xiaofeixiangqingName) {
					searchForm['xiaofeixiangqingName'] = this.searchForm.xiaofeixiangqingName
				}

				let res = await this.$api.list(`xiaofeixiangqing`, searchForm);
				// 如果是第一页数据置空
